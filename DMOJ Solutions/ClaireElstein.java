...
public class ClaireElstein {
	...
	public static void run(int testCaseNum) throws Exception {
		int n = in.readInt(), m = in.readInt(); long ans = 0;
		...
		int cnt[] = new int[n+1]; long dp[] = new long[n+1];
		for(int i=1; i<=n; i++)
			if(indegree[i] == 0) {
				q.add(i); cnt[i]=1; dp[i]=0;
			}
		
		while(!q.isEmpty()) {
			int u = q.poll();
			for(int e: adj.get(u)) {
				cnt[e] = (cnt[e] + cnt[u]) % MOD; indegree[e]--;
				dp[e] = (dp[e] + dp[u] + cnt[u]) % MOD;
				if(indegree[e] == 0) q.add(e);
			}
		}
		for(int i=1; i<=n; i++)
			if(adj.get(i).isEmpty()) ans = (ans + dp[i]) % MOD;
		out.println(ans);
	}
