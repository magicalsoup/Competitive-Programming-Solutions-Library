...
public class Paint {
	...
	public static void run(int testCaseNum) throws Exception {
		long n = in.readLong(), k = in.readLong(); interval[] a = new interval[(int)k];
		for(int i=0; i<k; i++) a[i] = new interval(in.readLong(), in.readLong());
		
		Arrays.sort(a); long ans = Integer.MIN_VALUE;
		
		TreeMap<Long, Long> dp = new TreeMap<Long, Long>();
		for(interval e: a) {
			Long l = dp.lowerKey(e.l);
			Long v = l == null? 0: dp.get(l);
			ans = Math.max(ans, v + e.r - e.l + 1);
			dp.put(e.r, ans);
		}
