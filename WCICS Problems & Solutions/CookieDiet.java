...
public class CookieDiet {
	...
	public static void run(int testCaseNum) throws Exception {
		int n = in.readInt(); long d = in.readLong(), t = in.readLong(), k = in.readLong();
		long s[] = new long[n+1]; for(int i=1; i<=n; i++) s[i] = in.readLong();
		
		boolean goodSanta = true;
		for(int i=1; i<=d; i++) {
			long total = 0, max = 0;
			for(int j=1; j<=n/d; j++) {
				total += s[j*i];
			...
			if(max > k || total >= t) goodSanta = false;
		}
		if(goodSanta == false)
			System.out.println("Naughty Santa");
		else
			System.out.println("Good Santa");
	}
