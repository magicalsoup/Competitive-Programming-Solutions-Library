...
public class LuckyBusiness {
	...
	public static void run(int testCaseNum) throws Exception {
		...
		int r[] = new int[n+1], e[] = new int[n+1];
		for(int i=1; i <=n; i++) {
			r[i] = in.readInt(); e[i] = in.readInt();
		}
		...
		for(int i=0; i<=n; i++) {
			for(int j=r[i]; j<=m; j++)
				dp[j]=Math.min(dp[j], dp[j-r[i]]+e[i]);
		}
		System.out.println(dp[m]== 0x3f3f3f3f3f3f3f3fL? -1: dp[m]);
	}
