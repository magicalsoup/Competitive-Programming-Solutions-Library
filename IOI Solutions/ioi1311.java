...
public class ioi1311{
	...
	public static void main(String[]args)throws IOException{
		...
		}// finshed first and second for loop;
		...
		for(Integer i:ends) {
			...
			starts.add(end);
		}//finished fourth for loop
		ArrayList<Integer> radii = new ArrayList<Integer>();
		visited = new boolean[n];
		for(int x=0;x<starts.size();x++)//very hard part, lots of for loops
			radii.add(getRadius(starts.get(x),diameters.get(x)));//first radii;
			int size =starts.size();
			Collections.sort(diameters,Collections.reverseOrder());
			Collections.sort(radii, Collections.reverseOrder());
			int minDist = diameters.get(0);// the min
			if(size>=2) {
	...
	public static int getRadius(int i,int diameter) {
		int curr =i;
		int total =0;
	...
	public static void getEnd(int i) {
		Stack<State> s = new Stack<State>();
		s.push(new State(i,0,0));
		while(!s.isEmpty()) {
			State curr = s.pop();
